...
simulate_election(100) %>%
  ...
  geom_density() +
  facet_wrap(~state) +
  labs(x = "Trump Vote Percentage", y = "Percentage", title = "Votes in 2020 Swing States")
...
```{r sc2}
elections <-
  tibble(simulations = map(rep(1, 1000), simulate_election)) %>%
   mutate(pa_win = map_lgl(simulations, ~.$Pennsylvania > 50)) %>%
  summarize(pa_avg = mean(pa_win))
  
  # mutate(pa_win = map_lgl(simulations, ~ifelse(.[[39]][1] > 50, TRUE, FALSE)))
